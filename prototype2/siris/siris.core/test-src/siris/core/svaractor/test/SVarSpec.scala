...
case class StartTest( sender : Actor )
case class TestFinished( sender : Actor, successful : Boolean )
case class OwnerChangeRetry[T]( sender: Actor, count: Int, sVar: SVar[T], a: Actor )
case class CheckValue[T]( sender: Actor, sVar: SVar[T] )
case class ValueChangeRetry[T]( sender: Actor, count: Int, sVar: SVar[T] )
case class IgnoreWait( sender: Actor, sVarIgnoreWaitCount: Int )
